  ...
  $.each(data, (i, c) => {
    $(".usuarios").append(`
              <tr>
                ...
                <td>
                  <button
                    class="btn btn-warning d-inline mx-1"
                    data-toggle="modal"
                    data-target="#exampleModal"
                    onclick="setInfoModal('${c.nombre}', '${c.balance}', '${c.id}')"
                  >
                    Editar</button
                  ><button class="btn btn-success d-inline mx-1" onclick="eliminarUsuario('${c.id}')">Eliminar</button>
                </td>
              </tr>
         `);
...
const eliminarUsuario = async (id) => {
  await fetch(`http://localhost:3000/usuario?id=${id}`, {
    method: "DELETE",
  });
